$authCookie = ".ASPXAUTH=; finviz_t=67d0f9db-96bc-4a7a-aae6-34b118f35ecc"
$headers = @{
    "Cookie" = $authCookie
    "User-Agent" = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"
}

$url = "https://finviz.com/quote.ashx?t=MRK"
$response = Invoke-WebRequest -Uri $url -Headers $headers -UseBasicParsing
$html = $response.Content

Write-Output "--- HTML LENGTH: $($html.Length) ---"
if ($html -match "Please solve the reCAPTCHA") { Write-Output "BLOCKED by CAPTCHA" }
if ($html -match "snapshot-td2") { Write-Output "Found snapshot-td2 class" }

# Show a snippet of the table if found
$index = $html.IndexOf("snapshot-td2")
if ($index -gt -1) {
    Write-Output "Snippet: $($html.Substring($index, 500))"
} else {
    Write-Output "snapshot-td2 NOT FOUND. Searching for any table data..."
    if ($html -match "<td") { Write-Output "Found some <td> tags" }
}
